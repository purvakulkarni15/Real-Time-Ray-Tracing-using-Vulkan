...
4. `Resizing operation can change this parameter on fly which is indicated to vulkan by setting the width and height in currentExtent to a special value: the maximum value of uint32_t. The resolution that best matches the window within the minImageExtent and maxImageExtent bounds is chosen.`???
